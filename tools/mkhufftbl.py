...
# This script reads the HPACK Huffman Code table from RFC 7541 Appendix B
# and generates the symbol encoding table and decoding FSM lookup tables
# used in src/hpack/huffman_data.cc.
#
# Original author: Tatsuhiro Tsujikawa (nghttp2)
# Adapted for LibHttp2: merged flags into fstate for compact representation.
#
# Reference: https://httpwg.org/specs/rfc7541.html#huffman.code
...
class Node:
    ...
    def __init__(self, term=None):
        self.term = term      # Symbol value if this is a leaf node
        self.left = None       # Left child (bit = 0)
        self.right = None      # Right child (bit = 1)
        self.trans = []        # 4-bit nibble transition table
        self.id = None         # Assigned node ID for internal nodes
        self.accept = False    # True if this state can terminate a valid sequence


class Context:
    """Builds and manages the Huffman decoding tree."""
...
def _add(node, sym, bits):
    """Insert a symbol into the Huffman tree following the bit path."""
    if len(bits) == 0:
        node.term = sym
        return
    if bits[0] == '0':
...
def _set_node_id(ctx, node, prefix):
    """Assign IDs to internal (non-leaf) nodes. Mark all-ones prefixes as accept states."""
...
def _traverse(node, sym, start_node, root, left):
    """Build the 4-bit nibble transition table for a single node."""
    if left == 0:
        if sym == 256:
            sym = None
            node = None
...
def _build_transition_table(ctx, node):
    """Recursively build transition tables for all internal nodes."""
    if node is None:
        return
...
# LibHttp2 flag constants (from huffman.h)
HTTP2_HUFF_ACCEPTED = 1 << 14
HTTP2_HUFF_SYM = 1 << 15


def _print_transition_table(node):
    """Print the decode table entries for all internal nodes.

    LibHttp2 format: {fstate, sym} where fstate packs the node ID and flags.
    """
    if node.term is not None:
        return
    ...
    for nd, sym in node.trans:
        ...
        if nd is None:
            ...
            if id is None:
                # Leaf node
                id = 0
                flags |= HTTP2_HUFF_ACCEPTED
            elif nd.accept:
                flags |= HTTP2_HUFF_ACCEPTED
        fstate = id | flags
        print('        {{0x{:x}, {}}},'.format(fstate, out))
    print('    },')
...
def huffman_tree_print_transition_table(ctx):
    ...
    print('    /* 256 -- terminal failure state */')
    print('    {')
    for _ in range(16):
        print('        {0x0100, 0},')
    print('    },')
...
if __name__ == '__main__':
    ...
    for line in StringIO(HUFFMAN_CODE_TABLE):
        ...
        if m:
            ...
            nbits = int(m.group(4))
            if len(code) > 8:
                raise ValueError('Code is more than 4 bytes long')
    ...
    # --- Output: huff_sym_table ---
    print('const http2_huff_sym huff_sym_table[] = {')
    for i in range(257):
        nbits = symbol_tbl[i][0]
        k = int(symbol_tbl[i][1], 16)
        k = k << (32 - nbits)
        comma = ',' if i < 256 else ''
        print('    {{{}, 0x{:08x}u}}{}'.format(nbits, k, comma))
    print('};')
    print()

    # --- Output: huff_decode_table ---
    print('const http2_huff_decode huff_decode_table[][16] = {')
